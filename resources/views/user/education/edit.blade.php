                            ...
                            <div class="row g-3 m-3">
                                    ...
                                    <div class="form-group {{ $errors->has('qualification_id') ? 'has-error' : '' }}">
                                        ...
                                        <select name="qualification_id" id="qualification_id" class="form-control select2">
                                            @foreach ($qualifications as $id => $qualification)
                                                <option value="{{ $id }}"
                                                    {{ (isset($education) && $education->qualification ? $education->qualification->id : old('qualification_id')) ==
                                                    $id
                                                        ? 'selected'
                                                        : '' }}>
                                                    {{ $qualification }}
                                    ...
                                    <div class="form-group {{ $errors->has('university_id') ? 'has-error' : '' }}">
                                        ...
                                        <select name="university_id" id="university_id" class="form-control select2">
                                            @foreach ($universities as $id => $university)
                                                <option value="{{ $id }}"
                                                    {{ (isset($education) && $education->university ? $education->university->id : old('university_id')) == $id
                                                        ? 'selected'
                                                        : '' }}>
                                                    {{ $university }}
                                                </option>
                                    ...
                                    <div class="form-group {{ $errors->has('division_id') ? 'has-error' : '' }}">
                                        ...
                                        <select name="division_id" id="division_id" class="form-control select2">
                                            @foreach ($divisions as $id => $division)
                                                <option value="{{ $id }}"
                                                    {{ (isset($education) && $education->division ? $education->division->id : old('division_id')) == $id
                                                        ? 'selected'
                                                        : '' }}>
                                                    {{ $division }}
                                                </option>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group {{ $errors->has('percentage') ? 'has-error' : '' }}">
                                        ...
                                        <input type="number" step="0.01" min="0.00" max="99.99" id="percentage"
                                            name="percentage" class="form-control" autocomplete="off" placeholder="0.00"
                                            value="{{ old('percentage', isset($education) ? number_format($education->percentage, 2) : '') }}">
                                    ...
                                    <div class="col-md-4">
                                        <div
                                            ...
                                            <div class="input-group">
                                                <input type="text" id="transcript_issue_date"
                                                    name="transcript_issue_date" class="form-control"
                                ...
                                <div class="col-md-6">
                                    <div class="form-group {{ $errors->has('transcript') ? 'has-error' : '' }}">
                                        ...
                                        @if ($education->media->where('media_type_id', 7)->isNotEmpty())
                                            <input type="hidden" id="old_transcript" name="old_transcript"
                                                value="{{ $education->media->where('media_type_id', 7)->first()->file_name }}"
                                                readonly>
                                        @endif
                                        ...
                                        @if (count($education->media))
                                            ...
                                            @foreach ($education->media->where('media_type_id', 7) as $mediaItem)
                                                {!! $mediaItem !!}
                                            @endforeach
                                        @endif
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group {{ $errors->has('character') ? 'has-error' : '' }}">
                                        ...
                                        <input type="file" class="form-control" id="character" name="character"
                                            ...
                                            style="display: block; border-color:#ccc">
                                        @if ($education->media->where('media_type_id', 6)->isNotEmpty())
                                            <input type="hidden" id="old_character" name="old_character"
                                                value="{{ $education->media->where('media_type_id', 6)->first()->file_name }}"
                                                readonly>
                                        @endif
                                        ...
                                        @if (count($education->media))
                                            {{ trans('global.existing_document.title_singular') }} :
                                            @foreach ($education->media->where('media_type_id', 6) as $mediaItem)
                                                {!! $mediaItem !!}
                                            @endforeach
                                        @endif
                                    </div>
