            ...
            $('input[name="samabeshi_groups[]"]').change(function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        // if (xhr.status === 422) {
                            $('#advAmount').val('0');
                            var errorMessage = '{{ trans('global.application.info.zeroAmountError') }}';
                            customErrorDiv.text(errorMessage);
                ...
                // Check for Ladies Error on selection by Male User
                var isLadiesChecked = selectedGroups.includes('2');
                // alert(isLadiesChecked);
                ...
                // Check for Samabeshi Document on Samabeshi Group Click except 1 and 2
                if (selectedGroups.some(function(group) { return group !== '1' && group !== '2'; })) {
                    var hasSamabeshiDocument = userSamabeshiMedia.length > 0;

                    if (!hasSamabeshiDocument) {
